...
| Showing `item://<id>` / `file://<id>` values to users | Opaque references — resolve them first (get items / `GET /v1/files/<id>/link`); driver-internal file reads use `ctx.reader.readFile` only when the driver declares `reads.files`; driver item write-backs use `ctx.writer.createItems` / `ctx.writer.updateItems` only when it declares `writes.items`; driver file write-backs use `ctx.writer.writeFile` only when it declares `writes.files` |
