...
**Drivers** are also installed per instance, not authored over REST: `yap driver add <npm-spec>` / `remove <name>` / `list`, into `YAP_DRIVERS_DIR` (default `./drivers`). A driver is trusted code running in-process — the SSRF guard defends hostile *parameters* through an honest driver, not a driver written to misbehave. Explicit surfaces are injected on the run context: `reads: { files: true }` gives `ctx.reader.readFile(refOrId)` for finalized files in the run bundle (`stream()` preferred; `bytes()`/`text()` capped), and `writes: { items: true }` gives `ctx.writer.createItems(...)` plus `ctx.writer.updateItems([{id, set?, edits?}])`; undeclared surfaces are `null`.
