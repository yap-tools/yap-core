/**
 * REST authentication. Bearer keys only: either a user access key or — on the
 * endpoints marked sysadmin — the environment-level sysadmin key. The
 * sysadmin key is not a user and holds no content capabilities.
 */
...
export async function requireUser(c: Context, db: Db, config: YapConfig): Promise<string> {
  ...
  if (!key) throw unauthorized("bearer access key required");
