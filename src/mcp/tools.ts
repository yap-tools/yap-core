...
Datatypes: text, number, boolean, date, plus item (a reference to another item in this bundle, item://<id>) and file (a reference to a finalized file, file://<id>). A property with multi: true holds an ordered list of values of its datatype (e.g. a multi text "tags" holds ["a","b"]); items then read/write that field as an array. config declares constraints, enforced on every write: text {pattern, enum} (enum restricts to a fixed set of strings; with multi it is a multi-select); number {min, max, decimals} (decimals default 2, out-of-precision writes rejected); item {itemType} (pin the referent's type); any multi field {minItems, maxItems}; single-valued text/number {unique} (no two items of the type may share a value — for ids and other identity fields). Properties can be renamed/added/removed later without touching stored data.`,
    parameters: z.object({
      space_id: z.string(),
      name: z.string(),
      description: z.string().optional(),
